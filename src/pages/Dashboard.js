...
const metrics = [
  { title: "Total Revenue", value: "₹45,231", change: "+12.5%", icon: DollarSign, color: "text-green-600", bgColor: "bg-green-50" },
  { title: "New Users", value: "2,400", change: "+3.1%", icon: Users, color: "text-blue-600", bgColor: "bg-blue-50" },
  { title: "Active Ledgers", value: "7", change: "+8.0%", icon: Target, color: "text-yellow-600", bgColor: "bg-yellow-50" },
];
...
const MetricCard = ({ title, value, change, icon: Icon, color, bgColor }) => (
  <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100 hover:shadow-xl transition">
    ...
    <div className="mt-4">
      ...
      <p className={`text-sm mt-1 font-semibold ${change && change.startsWith("+") ? "text-green-500" : "text-red-500"}`}>{change} since last month</p>
    </div>
  </div>
);
...
export default function Dashboard() {
  ...
  //           members: Array.from({ length: l.members }, (_, idx) => ({
  //             id: `U${idx + 1}`,
  //             username: `Member${idx + 1}`,
  //             email: "",
  //             amount: parseFloat(l.net_balance) / l.members || 0,
  //           })),
  //           transactions: [],
  ...
  const Sidebar = () => (
    <div
      className={`fixed inset-y-0 left-0 z-30 transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 transition-transform duration-300 ease-in-out w-64 bg-gray-900 md:relative md:flex md:flex-col md:h-full`}
    >
        ...
        {navBase.map((item) => (
          <button
            ...
            className={`group w-full text-left flex items-center px-3 py-2 text-sm font-medium rounded-lg transition ${
              view === item.id ? "bg-green-600 text-white shadow-md" : "text-gray-300 hover:bg-gray-800 hover:text-white"
            }`}
          >
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2 cursor-pointer" onClick={() => setView("profile")}>
            <img className="h-10 w-10 rounded-full border-2 border-green-500" src={fallbackAvatar} alt="user" />
  ...
  const DashboardContent = () => (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setIsFormOpen(true)} className="px-3 py-2 rounded-md bg-green-600 text-white">
          Create Ledger
        </button>
      </div>
              ...
              >
                ...
                <span
                  className={`px-3 py-1 text-sm rounded-full ${
                    l.status === "Active" ? "bg-yellow-100 text-yellow-800" : "bg-green-100 text-green-800"
                  }`}
                >
                  {l.status}
                </span>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto">
          ...
          {view === "ledgerDetails" && selectedLedger && (
            <div className="max-w-6xl mx-auto">
              <LedgerDetails ledgerId={selectedLedger.id} onBack={() => setView("dashboard")} />
            </div>
          )}
        </main>
      </div>
